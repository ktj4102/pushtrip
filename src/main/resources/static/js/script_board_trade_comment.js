$(document).ready(function(){
    console.log("boardTradeComment.js working!");

    // URL에서 tradeId 값 가져오기
    var urlParams = new URLSearchParams(window.location.search);
    var tradeId = urlParams.get('tradeId');
    console.log("Trade ID for CommentInsert :", tradeId);

    // tradeId 값을 input 필드에 설정
    $('#tradeId').val(tradeId);
    ...
    $(document).on('click', '.delete-btn', function() {
        var tradeCommId = $(this).data('id');
        console.log("comm ID for Comment Controller :", tradeCommId);
        if (confirm('정말로 이 댓글을 삭제하시겠습니까?')) {
            $.ajax({
                url: 'tradeForumCommentDelete', // 실제 삭제 요청 URL로 교체
                method: 'POST',
                data: { tradeCommId: tradeCommId },
                success: function(response) {
                    // 성공적인 응답 처리

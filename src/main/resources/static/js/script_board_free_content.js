...
$(document).ready(function(){
    ...
    var freeId = urlCommParams.get('freeId');
    console.log("Free ID for Comment Controller :", freeId);

    // AJAX 요청을 통해 댓글 데이터 가져오기
    $.ajax({
        url: "boardFreeComment", // 서버의 댓글 데이터 요청 URL
        type: "GET", // GET 또는 POST 요청 타입
        data: { freeId: freeId }, // 서버에 전송할 데이터
        success: function(data) {
            // 서버로부터 받은 데이터를 #freeComments에 로드
            $("#freeComments").html(data);
        },

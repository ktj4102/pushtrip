$(document).ready(function(){
    ...
    var questionId = urlParams.get('noticeId');
    console.log("notice ID for CommentInsert :", noticeId);

    // noticeId 값을 input 필드에 설정
    ...
    $(document).on('click', '.delete-btn', function() {
        var noticeCommId = $(this).data('id');
        console.log("noticeCommId for Comment Controller :", noticeCommId);
        if (confirm('정말로 이 댓글을 삭제하시겠습니까?')) {
            $.ajax({
                url: 'noticeForumCommentDelete',
                method: 'POST',
                data: { noticeCommId: noticeCommId },
                success: function(response) {
                    // 성공적인 응답 처리
